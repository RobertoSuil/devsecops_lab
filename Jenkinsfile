			...
			script{
			    ...
			    env.TARGET = ' http://zero.webappsecurity.com'
        		    ...
        		    sh "${DOCKER_EXEC} pull owasp/zap2docker-stable"
                                sh 'sudo ${DOCKER_EXEC} run --add-host="localhost:192.168.70.146" --rm -e LC_ALL=C.UTF-8 -e LANG=C.UTF-8 --name zap2 -u zap -p 8093:8093 -d owasp/zap2docker-stable zap.sh -daemon -port 8093 -host 0.0.0.0 -config api.disablekey=true'
                                sh 'sudo ${DOCKER_EXEC} run --add-host="localhost:192.168.70.146" -v /home/devsecops1313/Desktop/Jenkins/jenkins_home/tools:/zap/wrk/:rw --rm -i owasp/zap2docker-stable zap-full-scan.py -t "http://zero.webappsecurity.com" -I -r zap_full-scan_report.html'	
				        publishHTML([
        					allowMissing: false,
